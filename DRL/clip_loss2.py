...
def cos_layers(xs_conv_features, ys_conv_features, clip_model_name):
    ...
    #        zip(xs_conv_features, ys_conv_features)]
    if "RN" in clip_model_name:
        return [torch.square(x_conv, y_conv, dim=1).mean(1).mean(1).mean(1) for x_conv, y_conv in
                zip(xs_conv_features, ys_conv_features)]

    distances = []
    for x_conv, y_conv in zip(xs_conv_features, ys_conv_features):
        d = 1 - torch.cosine_similarity(x_conv, y_conv, dim=1).mean(1) #).mean(1).mean(1) 
        distances.append(d)
    return distances
...
class CLIPConvLoss(torch.nn.Module):
    ...
    def forward(self, sketch, target, mode="train"):
        ...
        #print("features: ", len(xs_conv_features), xs_conv_features[0].shape, xs_conv_features[11].shape)
        conv_loss = self.distance_metrics[self.clip_conv_loss_type](
            xs_conv_features, ys_conv_features, self.clip_model_name)
        #print("conv loss: ", len(conv_loss), conv_loss[0].shape)
...
clip_conv_layer_weights = [0.3, 0.3, 1.0, 1.0, 1.0]

a = {'clip_model_name':'ViT-B/32','clip_conv_loss_type':'Cos','device':device,
    'num_aug_clip':num_augs,'augemntations':['affine'],
     'clip_fc_loss_weight':0.1,'clip_conv_layer_weights':clip_conv_layer_weights}
