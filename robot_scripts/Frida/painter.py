...
class Painter():
    ...
    def __init__(self, opt, robot="franka", use_cache=False, writer=None):
        ...
        if robot == "franka":
            ...
            from lite6 import Lite6
            self.robot = Lite6(debug=True)
            self.robot_str = 'lite6'
        elif robot == None:
        ...
        if use_cache and os.path.exists(os.path.join(self.opt.cache_dir, "cached_params.pkl")):
            ...
            p = canvas_to_global_coordinates(PROP_MINMAX_W[0]+0.01, PROP_MINMAX_H[0]+0.01, self.opt.INIT_TABLE_Z, self.opt)
    ...
    def to_neutral(self, speed=0.4):
        # Initial spot
        if not self.opt.simulate:
            if self.robot_str == 'lite6':
                self.robot.zero_joints()
            else:
                self.robot.reset()
    ...
    def _move(self, x, y, z, q=None, timeout=20, method='direct', 
            step_size=.1, speed=0.1, duration=5, wait=False):
        ...
        self.robot.go_to_cartesian_pose([x,y,z], q, precise=wait)
        return None

    def hover_above(self, x,y,z, method='direct'):
        self._move(x,y,z+self.opt.HOVER_FACTOR, method=method, speed=0.4, wait=True)
        # rate = rospy.Rate(100)
        # rate.sleep()

    def move_to(self, x,y,z, q=None, method='direct', speed=0.05, wait=False):
        self._move(x,y,z, q=q, method=method, speed=speed, wait=wait)
    ...
    def get_paint(self, paint_index):
        ...
        self.move_to(x,y,self.Z_CANVAS) # z+self.opt.HOVER_FACTOR)
        ...
        orientations = [None]*len(positions)
        self.move_to_trajectories(positions, orientations, precise=True)
        self.move_to(x,y,self.Z_CANVAS + self.opt.HOVER_FACTOR,wait=True) # z+self.opt.HOVER_FACTOR)
        time.sleep(1)
        self.robot.rotate(0)
        #self.robot.reset()
        time.sleep(1)
    ...
    def set_height(self, x, y, z, move_amount=0.0015):
        ...
        '''
        move_amount = 0.5 #litle6 is in mm
        import getch
    ...
    def coordinate_calibration(self, debug=True, use_cache=False):
        ...
        homography_points = []
        for i in np.linspace(PROP_MINMAX_W[0]+0.1,PROP_MINMAX_W[1]-0.1,4): #(0.1, 0.9, 4):
            for j in np.linspace(PROP_MINMAX_H[0]+0.1,PROP_MINMAX_H[1]-0.1,4): #0.1, 0.9, 4):
                homography_points.append([i,j])
        
        i = 0
        for canvas_coord in homography_points:
            #TODO: set paint and water and rag positions
            if not self.opt.ink:
            ...
            all_strokes[stroke_ind]().paint(self, x_glob, y_glob, 0, wait=False)

        self.to_neutral()
        # Picture of the new strokes
        ans = input("Press Enter to move on to computing homography. Press 1 for reset")
        if ans == '1':
            self.to_neutral()
        ...
        print(real_coords_global.shape, sim_coords_global.shape)     
    ...
    def paint_extended_stroke_library(self):
        ...
        n_strokes_x = int(math.floor(self.opt.CANVAS_WIDTH/(self.opt.MAX_STROKE_LENGTH+20)))#0.005
        n_strokes_y = int(math.floor(self.opt.CANVAS_HEIGHT/(self.opt.MAX_BEND+20))) #0.01
        print("Number of strokes", n_strokes_x, n_strokes_y)
        input("Continue...")
        ...
        for paper_it in range(self.opt.num_papers):
            ...
            for y_offset_pix_og in np.linspace((PROP_MINMAX_H[0]+30/self.opt.CANVAS_HEIGHT), PROP_MINMAX_H[1] - (20/self.opt.CANVAS_HEIGHT), n_strokes_y)*h:
                #if n < 1: 
                #    n+=1
                #    continue 
                #for x_offset_pix in np.linspace(0.02, 0.99-(self.opt.MAX_STROKE_LENGTH/self.opt.CANVAS_WIDTH), n_strokes_x)*w:
                for x_offset_pix in np.linspace(PROP_MINMAX_W[0]+.02, PROP_MINMAX_W[1]-(self.opt.MAX_STROKE_LENGTH/self.opt.CANVAS_WIDTH), n_strokes_x)*w:
                    #print(y_offset_pix_og, x_offset_pix)
                    #input("next")
                    #continue
                    if len(stroke_trajectories) % 2 == 0:
                        ...
                        y_offset_pix = y_offset_pix_og + 5 * (h/self.opt.CANVAS_HEIGHT) # Offset y by 1cm every other stroke
                    else:
                        y_offset_pix = y_offset_pix_og

                    temp_offsets = (x_offset_pix,y_offset_pix)
                    x, y = x_offset_pix / w, 1 - (y_offset_pix / h)
                    #x, y = min(max(x,0.),1.), min(max(y,0.),1.) #safety
                    x, y = min(max(x,PROP_MINMAX_W[0]),PROP_MINMAX_W[1]), min(max(y,PROP_MINMAX_H[0]),PROP_MINMAX_H[1])
                    x_offset_pix =(1-y)*w #(min(x,PROP_MINMAX[0]))* w 
                    y_offset_pix = (x)*h
                    temp_props = (x,y)
                    ...
                    if len(stroke_trajectories) < len(random_strokes):
                        ...
                        print("Got Preprogramed Stroke", random_strokes[len(stroke_trajectories)])
                    else:
                        random_stroke = get_random_stroke()
                        print("Got Random Stroke", random_stroke)
                    #input("Painting Next Random Stroke: " + str(x) + ", " + str(y))
                    #random_stroke.angled_paint(self, x, y, 0)
                    #input("Continue Painting")
                    random_stroke.paint(self, x, y, 0, wait=True)

                    self.to_neutral()                    
                    time.sleep(1)

                    ans = input("Take Photo. To reset, press 1: ")
                    while ans == '1':
                        self.to_neutral()
                        ans = input("Take Photo. To reset, press 1: ")
                    ...
                    #print(temp_props, temp_offsets, x_offset_pix, y_offset_pix)
                    #show_img(stroke)
