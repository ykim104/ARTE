...
class Paint:
    ...
    def load_data_mnist(self):
        if os.path.exists('./data/img_train_mnist.pkl.pkl') and os.path.exists('./data/img_test_mnist.pkl.pkl'):
            self.img_train = pickle.load(open("img_train_mnist.pkl", "rb"))
            self.img_test = pickle.load(open("img_test_mnist.pkl", "rb"))
        ...
        self.img_train = mnist_trainset
        self.img_test = mnist_testset
        self.train_num = len(self.img_train)
        self.test_num = len(self.img_test)

        pickle.dump( self.img_train, open( "./data/img_train_mnist.pkl", "wb" ) )
        pickle.dump( self.img_test, open( "./data/img_test_mnist.pkl", "wb" ) )
                ...
                try:
                    ...
                    if i > 2000:                
                        ...
                        if self.loss_fcn == 'cm' or self.loss_fcn == 'cml1':
                            ...
                            #print(mask.shape, img.shape, len(self.img_train), len(self.mask_train))
                        ...
                        if self.loss_fcn == 'cm' or self.loss_fcn == 'cml1':
                            ...
                            mask = mask.numpy() * 255
                            mask = mask.astype(np.uint8)
                            #print(mask.shape, img.shape, len(self.img_test), len(self.mask_test), type(img[0,0,0]), type(mask[0,0,0]))
    ...
    def load_data_sketchy(self):
        ...
        pickle.dump( self.img_train, open( "img_train_sketchy.pkl", "wb" ) )
        pickle.dump( self.img_test, open( "img_test_sketchy.pkl", "wb" ) )
        if self.loss_fcn == 'cm' or self.loss_fcn == 'cml1':
            pickle.dump( self.mask_train, open( "mask_train_sketchy.pkl", "wb" ) )
            pickle.dump( self.mask_test, open( "mask_test_sketchy.pkl", "wb" ) )
        print('finish loading data, {} training images, {} testing images'.format(str(self.train_num), str(self.test_num)))
        
    def pre_data(self, id, test):
        if self.dataset == 'sketchy':
            self._env_count += 1
            if self._env_count % 100000 == 0 and self.dilation_rate>=2: #200000
            ...
            # dilate
            image_larger = cv2.resize(img,(256,256))
            image_larger = 1-image_larger #img_dilation     
            kernel = np.ones((self.dilation_rate, self.dilation_rate), np.uint8)            
            img_dilation = cv2.dilate(image_larger, kernel, iterations=self.dilation_rate)
            img_dilation = cv2.resize(img_dilation, (128,128))
            #img_dilation2 = cv2.dilate(1-img, kernel, iterations=self.dilation_rate)
            img = 1-img_dilation
            #cv2.imshow("image", np.hstack((1-img_dilation1,1-img_dilation2)))
            #cv2.waitKey(0)
        ...
        if self.dataset == 'mnist':
            if test:
                img, target = self.img_test[id]
            else:
                img, target = self.img_train[id]
            img = img.convert('RGB')
            img = np.asarray(img)
            img = cv2.resize(img, (width, width))
    ...
    def reset(self, test=False, begin_num=False, step=None):
        self.test = test
